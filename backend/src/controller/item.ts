...
export const updateItem = async (req: any, res: Response) => {
  try {
    ...
    if (!whatsapp && !facebook && !tiktok && !instagram) {
    ...
    if (req.files && (req.files as any[]).length > 0) {
      const uploadPromises = (req.files as any[]).map(file => 
        cloudinary.uploader.upload(file.path, { folder: 'tradara_marketplace' })
      );
      const results = await Promise.all(uploadPromises);
      updatedImages = results.map(r => r.secure_url);
    }
    ...
    const updated = await prisma.item.update({
      ...
      data: { 
        ...
        city: cleanStr(city), 
        area: cleanStr(area), 
        country: cleanStr(country) || "Nigeria", 
        category, 
        description, 
        currency,
        whatsapp: cleanStr(whatsapp),
        ...
        tiktok: cleanStr(tiktok),
        instagram: cleanStr(instagram),
        images: updatedImages ? updatedImages : undefined,
        canBargain: canBargain === 'true' || canBargain === true,
...
export const deleteItem = async (req: any, res: Response) => {
  try {
    await prisma.item.delete({ where: { id: req.params.id, userId: req.user.id } });
