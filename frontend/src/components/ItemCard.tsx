...
import { 
  ...
  MessageSquare, Instagram, Facebook, Video, X, Bot, Sparkles
} from 'lucide-react';
...
export const ItemCard: React.FC<ItemCardProps> = ({ item }) => {
  ...
  const handleAskAi = (e: React.MouseEvent) => {
    ...
    const productId = item._id || item.id;
    if (productId) {
      navigate(`/product/${productId}?openAiChat=true`);
    }
  };
      ...
      <div onClick={handleViewDetails} className="glass-card group rounded-[2.5rem] overflow-hidden hover:scale-[1.03] transition-all duration-500 hover:border-blue-500/40 cursor-pointer flex flex-col h-full relative">
        <div className="relative aspect-[4/5] overflow-hidden bg-slate-900 shrink-0">
          ...
          <div className="absolute top-3 left-3 flex flex-col gap-1">
             ...
             {item.aiConfig && (
               <div className="backdrop-blur-xl bg-indigo-600/30 border border-indigo-400/40 px-2.5 py-0.5 rounded-full flex items-center gap-1.5">
                  <Bot size={10} className="text-indigo-300" />
                  <span className="text-[8px] font-black text-indigo-300 uppercase tracking-tighter flex items-center gap-0.5">
                    AI Enabled <Sparkles size={8} />
                  </span>
               </div>
             )}
          </div>
